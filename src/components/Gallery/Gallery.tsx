...
const Gallery = ({ galleryData }: any) => {
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 xxl:grid-cols-4 gap-34`}>
      <GalleryImagesList imagesList={galleryData} onImageClick={handleSelectDisplayedImg} />

      {isImgPopupOpen && (
        <GallerySliderWrapper
          className={`fixed hidden md:flex justify-center items-center top-0 right-0 bottom-0 left-0 z-10`}
          onClick={() => handleShowGelleryPopup()}>
          <GallerySliderContent className={`relative z-20`} onClick={(e) => e.stopPropagation()}>
            <ExitButton
              className={`absolute block top-0 text-20 cursor-pointer text-white bg-transparent border-0`}
              onClick={() => handleShowGelleryPopup()}>
              X
            </ExitButton>
            <Slider initialSlide={slideIndex} {...imageSliderSettings} className="gallerySlider">
              {!!galleryData?.length &&
                galleryData.map(
                  (
                    {
